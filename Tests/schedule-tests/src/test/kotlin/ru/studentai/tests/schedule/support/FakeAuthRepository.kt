...
internal class FakeAuthRepository(
    ...
    private val authStateFlow: MutableStateFlow<AuthState> = MutableStateFlow(initialAuthState)
    override val authState: Flow<AuthState> = authStateFlow

    var loginResult: DomainResult<User> = DomainResult.Success(ScheduleFixtures.studentUser())
    var registerResult: DomainResult<User> = DomainResult.Success(ScheduleFixtures.studentUser())
    var logoutResult: DomainResult<Unit> = DomainResult.Success(Unit)
    var currentProfileResult: DomainResult<UserProfile> =
        DomainResult.Success(ScheduleFixtures.studentProfile())
